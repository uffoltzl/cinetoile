...
function printHand() {
  ...
  if (getDifficulty() != 0) {
    ...
    document
      .getElementById("real_button_clue")
...
function update() {
  if (nbError >= 3 || (cards.length === 0 && hand.length === 0)) {
    ...
    if (getDifficulty() != 0) {
      document
        .getElementById("real_button_clue")
        .removeEventListener("click", click_clue_button);
    }
  ...
  if (i < 3) {
    ...
    document.querySelectorAll(".image_undone").forEach((image) => {
      image.style.animation =
        "vibrate 0.50s cubic-bezier(.36,.07,.19,.97) 2 both";
    });
    var ind = document.getElementById("indication");
    ind.style.opacity = 1;
    ind.style.animation =
      "typing 3.5s steps(40, end), cursor .75s step-end infinite";
  }
}
...
function click_clue_button() {
  if (nbClues > 0) {
    ...
    document.getElementById("clues").style.animation =
      "vibrate 0.50s cubic-bezier(.36,.07,.19,.97) 2 both";
  }
}
